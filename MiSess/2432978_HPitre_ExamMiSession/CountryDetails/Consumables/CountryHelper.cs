...
namespace CountryDetails.Consumables
{
    internal class CountryHelper
    {
        private static readonly HttpClient client = new()
        {
            BaseAddress = new Uri(ConfigHelper.APIBaseUrl)
        };

        #region GET  Methods
        public static async Task<RESTCountry?>? GetAsync(string name)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync(name + "?fields=capital,region,subregion,population,area");
                if (!response.IsSuccessStatusCode) return null;//If response = 200, return true, else false

                string json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<RESTCountry[]>(json,
