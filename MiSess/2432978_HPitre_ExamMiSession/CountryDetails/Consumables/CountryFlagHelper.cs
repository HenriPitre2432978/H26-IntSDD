...
{
    internal class CountryFlagHelper
    {
        ...
        #region GET  Methods
        public static async Task<RESTFlag?>? GetAsync(string name)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync(name + "?fields=flags");
                if (!response.IsSuccessStatusCode) return null;//If response = 200, return true, else false

                string json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<RESTFlag[]?>(json,
