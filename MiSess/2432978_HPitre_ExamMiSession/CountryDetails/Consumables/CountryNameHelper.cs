...
{
    internal class CountryNameHelper
    {
        ...
        #region GET  Methods
        public static async Task<RESTName?>? GetAsync(string name)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync(name + "?fields=name");
                if (!response.IsSuccessStatusCode) return null;//If response = 200, return true, else false

                string json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<RESTName[]?>(json,
